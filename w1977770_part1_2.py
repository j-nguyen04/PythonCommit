...
while state:
    ...
    if pass_credits + defer_credits + fail_credits == 120:
        ...
        student_credits = [pass_credits, defer_credits, fail_credits]
        
        if pass_credits == 120:
            print('Progress')
            progress_counter += 1
            progress_list_data.append(student_credits)

        elif pass_credits == 100:
            print('Progress (module trailer)')
            trailer_counter += 1
            trailer_list_data.append(student_credits)
    
        elif fail_credits >= 80:
            print('Exclude')
            exclude_counter += 1
            exclude_list_data.append(student_credits)

        else:
            print('Do not Progress - module retriever')
            retriever_counter += 1
            retriever_list_data.append(student_credits)
    ...
    while True:
        ...
        if user_input == 'q':
            ...
            for i in range(len(progress_list_data)):
                credits = progress_list_data[i]
                print(f"Progress - {credits[0]}, {credits[1]}, {credits[2]}")

            for i in range(len(trailer_list_data)):
                credits = trailer_list_data[i]
                print(f"Progress (module trailer) - {credits[0]}, {credits[1]}, {credits[2]}")

            for i in range(len(exclude_list_data)):
                credits = exclude_list_data[i]
                print(f"Exclude - {credits[0]}, {credits[1]}, {credits[2]}")

            for i in range(len(retriever_list_data)):
                credits = retriever_list_data[i]
                print(f"Module retriever - {credits[0]}, {credits[1]}, {credits[2]}")
